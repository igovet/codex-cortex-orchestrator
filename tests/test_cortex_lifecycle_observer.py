...
def _fixture(tmp_path: Path) -> tuple[dict[str, str], Path]:
    ...
    candidate = codex / "plugins/cache/cortex/cortex/1.14.1"; candidate.mkdir(parents=True, mode=0o700)
    nonce = "a" * 64; build = "sha256:" + "b" * 64
    request_generation(code_home=codex, build_id=build, candidate_version="1.14.1", catalogue_count=15, catalogue_digest="c" * 64, session_nonce=nonce)
    consume_intent(code_home=codex, package_root=candidate, build_id=build, candidate_version="1.14.1", catalogue_count=15, catalogue_digest="c" * 64, session_nonce=nonce)
    claim_generation(package_root=candidate, build_id=build, candidate_version="1.14.1", catalogue_count=15, catalogue_digest="c" * 64, session_nonce=nonce)
