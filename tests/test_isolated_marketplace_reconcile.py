...
def test_stale_cortex_source_is_replaced_without_touching_unrelated_marketplaces(tmp_path: Path) -> None:
    state = {
        ...
        "installed_version": "1.14.9+codex.sha256.stale0000000000",
    }
    ...
    assert after["installed_version"].startswith("1.14.9+codex.sha256.")
...
def test_install_rebuilds_when_checkout_manifest_suffix_is_stale(tmp_path: Path) -> None:
    ...
    payload["version"] = "1.14.9+codex.sha256.0000000000000000"
    ...
    try:
        ...
        staged = list((tmp_path / "owner/.cortex-dev/.codex/.cortex-candidates").glob("1.14.9+codex.sha256.*"))
        assert len(staged) == 1
