...
class V17MaintenanceSchemaTests(unittest.TestCase):
    ...
    def _healthy_v17_store(self) -> tuple[V12Store, str]:
        ...
        task, _ = store.create_task(
            ...
            outcome_contracts=[{'requirement': 'Retain v17 relation columns.', 'acceptance': ['Report missing immutable relation columns.'], 'verification': ['Inspect maintenance schema health.'], 'constraints': []}],
        ...
        for column in _PLAN_REVIEW_RELATION_COLUMNS:
            with self.subTest(column=column), tempfile.TemporaryDirectory(
                ...
                task, _ = store.create_task(
                    ...
                    user_language="en",
                    task_contract_version="cortex/task-contract/v3-outcome-linked",
                    outcome_contracts=[{'requirement': 'Validate all v17 relation columns.', 'acceptance': ['Fail closed with safe maintenance code.'], 'verification': ['Run read-only maintenance health.'], 'constraints': []}],
