...
SCHEMA_VERSION = 1
BASE_VERSION = "1.15.3"
_STAMPED_VERSION = re.compile(r"^1\.15\.3\+codex\.sha256\.([0-9a-f]{16})$")
...
def _candidate_path(codex_home: Path, version: str) -> Path:
    if not _STAMPED_VERSION.fullmatch(version):
        raise CandidateReceiptError("candidate version is not the required content-addressed 1.15.3 stamp")
