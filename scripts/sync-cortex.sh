...
if module.SERVER_VERSION != version or base_version != "10.0.5":
    raise SystemExit("plugin/server version must match the 10.0.5 release manifest")
PY
}
...
sync_cortex_hook_trust() {
  ...
  if [[ "${mode}" == "dry-run" ]]; then
    echo "would trust the six exact installed Cortex lifecycle hook content hashes"
    return 0
  fi
  if [[ "${mode}" == "check" ]]; then
