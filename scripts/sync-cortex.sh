...
PY
  ...
  [[ "${candidate_version}" =~ ^1\.14\.9\+codex\.sha256\.[0-9a-f]{16}$ ]] || {
    rm -rf -- "${temporary}"
...
if module.SERVER_VERSION != base_version or base_version != "1.14.9":
    raise SystemExit("plugin/server semantic version must match the 1.14.9 release manifest")
PY
}
