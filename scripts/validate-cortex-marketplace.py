...
VERSION_PATTERN = re.compile(r"^1\.14\.0\+codex\.sha256\.[0-9a-f]{16}$")
EXPECTED_SKILLS = (
    "adaptive-pipeline",
    "content-safety",
    "context-compaction",
...
def validate_manifest(plugin: Path, *, candidate: bool = False) -> None:
    ...
    if manifest.get("name") != EXPECTED_PLUGIN or not isinstance(version, str) or not valid_version:
        fail("installable plugin manifest must use a content-addressed 1.14.0 version")
    if version.split("+", 1)[0] != EXPECTED_BASE_VERSION:
        fail("plugin manifest semantic version must be 1.14.0")
