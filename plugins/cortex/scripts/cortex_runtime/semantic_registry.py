...
_ERROR_SPECS: tuple[ErrorSpec, ...] = (
    ...
    ErrorSpec("assignment_not_ready", "The selected work has unmet typed prerequisites or conflicting ownership.", "Read current scope and its readiness reasons. Satisfy the existing baseline, dependency or reconciliation boundary before assigning; changing model or effort cannot make waiting work ready."),
    ...
    ErrorSpec("recovery_state_required", "The fresh coordinator has not established current task state.", "Open a new task or read the existing task state before other coordinator operations."),
    ErrorSpec("recovery_continuations_required", "The recovered coordinator has not consumed its current continuation view.", "Read active continuations for the recovered task and consume every bounded page before any other task progress operation."),
    ...
    ErrorSpec("closure_review_required", "A current user closure review must be recorded before advisory closure.", "Present the reconciled result, open the mandatory closure review, and record the user's explicit revise or close choice before retrying close_task."),
    ErrorSpec("closure_revision_requested", "The user requested revision of the current task during closure review.", "Keep the task open and route bounded in-contract rework; present a fresh result and closure review after the new evidence."),
    ErrorSpec("closure_review_stale", "The accepted closure review is stale after later task activity.", "Reconcile the new evidence, present the updated result, and open and record a fresh closure review before retrying close_task."),
)
...
_GENERIC_PUBLIC_ERROR_CODES = frozenset({
    ...
    "invalid_closure_subject", "invalid_decision_type",
