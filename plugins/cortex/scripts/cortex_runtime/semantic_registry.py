...
_ERROR_SPECS: tuple[ErrorSpec, ...] = (
    ...
    ErrorSpec("assignment_loss_scope_conflict", "The selected outcomes do not identify one exact recoverable predecessor.", "Read current task state and, only with confirmed loss evidence, copy one exact loss_recovery_outcomes list without adding or removing outcomes; do not replay or guess assignment lineage."),
...
_SPECS: tuple[OperationSpec, ...] = (
    ...
    OperationSpec("read_task", "query", ("task-projection", "timeline", "contract-revision"), "read_task", "read_task", ("task_state",), ("task_ref",), handler_name="read_task", safe_errors=("task_not_found", "report_cursor_invalid", "assignment_not_consumed", "wrong_connection", "connection_lost", "assignment_stale")),
