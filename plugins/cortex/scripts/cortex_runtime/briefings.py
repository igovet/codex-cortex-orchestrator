...
def host_spawn_prompt(agent: str, package: dict[str, Any]) -> str:
    ...
    if package.get("facade_managed"):
        ...
        lifecycle_contract = (
            ...
            f"`validate_report_draft` with this identity and draft_ref only. Its report has {report_contract}; use [] "
            "when empty. If the host sandbox cannot edit draft_path, send one complete replacement once or a small "
            "JSON Merge Patch through validate_report_draft. "
            ...
            "for every caller-correctable diagnostic by editing the same file or sending a small patch until "
            "draft_valid=true; invalid validations keep the draft and consume no worker attempt. Then call "
            "record_report once with only the exact worker identity, draft_ref, and validation_digest; do not resend, "
            "rebuild, or reconsider the report payload. It reads and deletes that same file only after successful "
            "persistence. If atomic revalidation reports a caller-correctable state change, return to draft "
        ...
        lifecycle_contract = (
            ...
            "placeholder, and validate that same draft_ref repeatedly until draft_valid=true. If the sandbox cannot "
            "edit the file, use a small validation patch. Invalid validations keep the draft and consume no worker "
            "attempt. Then publish exactly one cortex/report/v1 report for "
            "this attempt by calling record_report with only worker identity, draft_ref, and validation_digest; "
            "never resend or reconstruct the report payload; the final tool deletes the same file after success. "
