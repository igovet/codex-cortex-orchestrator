...
def host_spawn_bootstrap(
    ...
    return (
        ...
        "Read exceptions: issued briefing "
        f"{str(briefing_path)!r}, intent={str(intent_path or '')!r} sha256={str(intent_digest or '')!r}; "
        f"plan={str(plan_unit_path or '')!r} sha256={str(plan_unit_digest or '')!r}. Digest mismatch blocks. "
...
def _compact_assignment_for_transport(assignment: dict[str, Any], *, aggressive: bool = False) -> None:
    ...
    if isinstance(decisions, list):
        ...
        for item in decisions[-4:]:
            if not isinstance(item, dict):
                continue
            question = str(item.get("question_en") or item.get("question") or "").strip()[:240]
            answer = str(item.get("answer_en") or item.get("answer") or "").strip()[:320]
            if question or answer:
                compact_decisions.append({"question_en": question, "answer_en": answer})
        assignment["resolved_user_decisions"] = compact_decisions
    ...
    if isinstance(governance, dict):
        ...
        for key in (
            ...
            if value not in (None, ""):
                compact_governance[key] = str(value)[:240]
        for key in ("reasons", "trigger_evidence", "close_obligations", "current_pipeline"):
            values = governance.get(key)
            if isinstance(values, list):
                compact_governance[key] = [str(item).strip()[:180] for item in values[:4] if str(item).strip()]
    ...
    bounded_fields = {
        "requirements": (4, 300),
        "scope": (4, 120),
        "allowed_paths": (12, 160),
        "context_files": (4, 120),
        "knowledge_index_files": (3, 120),
        "predecessor_result_refs": (6, 80),
        "gate_acceptance_criteria": (4, 300),
        "gate_verification": (4, 300),
        "task_acceptance_criteria": (4, 300),
        "task_verification": (4, 300),
    }
    for field, (limit, item_chars) in bounded_fields.items():
        value = assignment.get(field)
        if isinstance(value, list):
            assignment[field] = [str(item).strip()[:item_chars] for item in value[:limit] if str(item).strip()]
            ...
            compiled_context["decisions"] = [
                {
                    ...
                    "answer": str(item.get("answer") or "").strip()[:320],
                }
    ...
    if aggressive:
        ...
        for field, limit, item_chars in (
            ("requirements", 2, 180), ("scope", 2, 80),
            ("context_files", 2, 100), ("knowledge_index_files", 2, 100),
            ("gate_acceptance_criteria", 2, 180), ("gate_verification", 2, 180),
            ("task_acceptance_criteria", 2, 180), ("task_verification", 2, 180),
        ):
            value = assignment.get(field)
            if isinstance(value, list):
                assignment[field] = [str(item).strip()[:item_chars] for item in value[:limit] if str(item).strip()]
        paths = assignment.get("allowed_paths")
        if isinstance(paths, list):
            assignment["allowed_paths"] = [str(item).strip()[:120] for item in paths[:8] if str(item).strip()]
        refs = assignment.get("predecessor_result_refs")
        if isinstance(refs, list):
            assignment["predecessor_result_refs"] = [str(item).strip()[:64] for item in refs[:8] if str(item).strip()]
        decisions = assignment.get("resolved_user_decisions")
        if isinstance(decisions, list):
            assignment["resolved_user_decisions"] = [
                {
                    "question_en": str(item.get("question_en") or "")[:120],
                    "answer_en": str(item.get("answer_en") or "")[:160],
                }
        ...
        if isinstance(governance, dict):
            for key in ("reasons", "trigger_evidence", "close_obligations", "current_pipeline"):
                values = governance.get(key)
                if isinstance(values, list):
                    governance[key] = [str(item).strip()[:80] for item in values[:2] if str(item).strip()]
        if isinstance(compiled_context, dict):
        ...
        if isinstance(mission, str):
            assignment["mission"] = mission[:120]
        intent = assignment.get("user_intent")
        if isinstance(intent, dict):
            intent["projection"] = str(intent.get("projection") or "")[:160]
        ...
        if isinstance(governance, dict):
            assignment["governance_context"] = {
                key: governance[key]
                for key in ("schema", "requested_mode", "effective_mode", "complexity", "policy_snapshot_digest")
...
def host_spawn_prompt(agent: str, package: dict[str, Any]) -> str:
    ...
    assignment = {
        ...
        "plan_feedback": plan_feedback[:1200] or None,
    ...
    if gate == "governance_close":
        ...
        assignment["budget"] = str(assignment.get("budget") or "")[:240] or None
    ...
    context_parts = [
        ...
        "Only that exact read-only intent path, an optional compiled-plan path, listed context files, and listed predecessor results are authorized reads. "
