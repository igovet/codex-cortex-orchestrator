...
def build_public_contracts() -> dict[str, dict[str, Any]]:
    ...
    plan_publication = _closed({
        ...
        **publication_evidence,
    }, ("task_ref", "status", "summary", "scope", "review_policy", "stages", "verification_facts", "outcome_coverage", "risks", "unresolved"), description="Flat closed plan publication derived from this connection's assignment. Every required property must be present; when there are no risks or unresolved items, supply the corresponding empty arrays.")
    ...
    contracts = {
        ...
        "read_task": _contract("Single bounded coordinator or worker read and the only operation that reads or consumes a worker assignment. A fresh worker's first Cortex operation is read_task with its assignment view using the exact server-rendered worker-scoped task_ref; never use open_assignment, which is coordinator-only creation. Coordinator state binds every exact current semantic outcome to its coverage, ownership, and delivery-assignability disposition; use that mapping rather than row position when selecting assignment scope after steering. Omitted or false continuation starts a read; continue only the immediately preceding identical read when its result says more data exists. A fully consumed identical read is reconciled without minting another receipt. The server owns position, so never invent cursors, offsets, continuation handles, or identity fields.", _closed({
            "task_ref": task_ref,
        ...
        "open_assignment": _contract("Coordinator-only creation of exactly one private worker assignment from the advertised flat fields. This operation never reads or consumes an existing worker assignment: a fresh worker must use read_task with its assignment view and must never call open_assignment. Immediately before the coordinator call, read current task state and copy complete exact unique selected outcome names from that current effective contract. For ordinary delivery select only outcomes whose exact state disposition says delivery_assignability=assignable; never include terminal-owned outcomes. For confirmed loss recovery copy one complete loss_recovery_outcomes list from one current state item without adding, removing, or combining outcomes. On an explicit stale-current-outcome rejection, at most one fresh-state read and one materially rebuilt complete request are permitted when the intended scope maps unambiguously; never replay the stale request or reconstruct retired identity. The coordinator owns routing, responsibility, scope, complete instructions, semantic outcome selection, and predecessor evidence policy. Goal or instructions never replace the required outcome selection. Instructions are the sole task-specific instruction field; never invent supplementary fields, nested mission objects, or caller-side lineage. For a confirmed lost nonterminal delivery worker only, loss_recovery must explicitly record blocked or aborted state, a concrete reason, and non-empty evidence; Cortex derives the unique predecessor from the exact current outcomes and atomically links the successor. Never infer loss from timeout, reconnect, a copied locator, report reference, or bare assignment reference. A planning assignment may publish only one terminal plan. Forward a successful non-replayed native dispatch immediately and exactly once; a replay or ambiguous mutation must not spawn another worker.", _closed({
            ...
            "outcomes": outcome_names, "report_policy": assignment_report_policy,
