...
def build_public_contracts() -> dict[str, dict[str, Any]]:
    ...
    contracts = {
        ...
        "publish_plan": _contract("Worker-only atomic terminal plan publication. Publish one complete plan; risks and unresolved arrays may be empty. Cortex derives review from the coordinator's authoritative governance assessment and complete plan evidence: only minimal, risk-free, complete work may be informational; light/full, uncertainty, risks, or incomplete evidence requires review regardless of language. The planner cannot choose or downgrade that policy. Publish once, then stop; changed content requires a new assignment.", plan_publication, receipt),
