...
def verify_runtime(package_root: Path, server_version: str, environment: Mapping[str, str] | None = None, *, allow_source_mode: bool = False) -> dict[str, str]:
    ...
    match = re.fullmatch(r"(1\.12\.3)\+codex\.sha256\.([0-9a-f]{16})", version)
    ...
    source_mode = allow_source_mode and (version == "1.12.3" or match is not None)
    if (match is None and not source_mode) or server_version != "1.12.3":
