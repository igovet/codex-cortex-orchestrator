...
WORKER_CONTEXT = (
    ...
    "record_report after all blocking questions are answered. Build from the current template and repeat draft validation on the same task and attempt until draft_valid=true; draft checks persist nothing and consume no worker attempt. Then call record_report once with the exact unchanged payload and returned validation_digest. After it succeeds, return only REPORT_RECORDED report_ref=<value> plus at most a two-sentence "
