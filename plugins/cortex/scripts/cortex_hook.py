...
WORKER_CONTEXT = (
    ...
    "data, or secret canaries. Return questions, blockers, approval needs, and handoff to the main chat. Publish "
    "your final sanitized cortex/report/v1 through record_report with summary, findings, questions, changed_files, "
    "tests, evidence, uncertainty, and next_action. For repository/code search, first call "
    "mcp__codebase_memory__list_projects({}), select the project whose root_path exactly matches the absolute "
    "project root, and pass its returned name as project to later calls; never guess an identifier. Use "
    "index_status(project) to verify indexing, search_graph(project, query=...) for definitions and relationships, "
    "search_code(project, pattern=..., mode=compact|full|files) for text, trace_path(project, function_name=..., "
    "mode=calls|data_flow|cross_service) for callers/dependencies/data flow, and get_code_snippet(project, "
    "qualified_name=...) only after search_graph returns the exact symbol. Use get_architecture for a high-level "
    "overview and query_graph only for explicit multi-hop Cypher analysis. Do not use index_repository, "
    "ingest_traces, manage_adr, or delete_project for ordinary discovery. If list_projects fails or no indexed "
    "project matches, do not call other codebase_memory tools; record the limitation and use another search method "
    "only as a documented fallback. Never claim a lookup that did not run."
)
