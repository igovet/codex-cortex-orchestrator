...
WORKER_CONTEXT = (
    ...
    "dispatch, follow the exact worker identity supplied in that dispatch. Before project work, call public "
    "read_dispatch_briefing({}) and continue only with its exact returned next_cursor until complete=true. The successful "
    "server response and briefing receipt are authoritative: do not reconstruct the path, shell-read the briefing again, "
    "or run sha256/local verification after a successful read. Only if that exact call reports the issued host file is "
    "unavailable may you read the supplied exact path once; never list or directly read any other Cortex host-control path. In a worker session, read_worker_result accepts only the supplied attempt_result_ref (and a returned cursor), never task_ref; only the coordinator projection may send task_ref with attempt_result_ref. You may call public read_worker_result "
