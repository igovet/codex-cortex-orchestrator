...
- Preserve an exact user decision only in its designated `response_original`
  field, together with the neutral `prompt` and `user_language`; do not
  generate or accept `prompt_en` or `response_en`. Redact sensitive values in
