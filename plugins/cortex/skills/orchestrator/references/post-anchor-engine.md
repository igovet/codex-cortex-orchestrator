---
name: orchestrator
description: Explicit opt-in Cortex v1.12.1 coordinator for worker-only project execution, closed exact-path knowledge routing, durable evidence handoff, and model-owned advisory governance. Use only when the user directly selects or mentions cortex:orchestrator; use the host-supplied skill context and never fetch skill URIs through MCP resources/read.
---
...
Call ownership is strict. The coordinator never publishes a worker outcome, whether
...
retry identity. Do not retry schema, size, reference, safety, or idempotency
...
question covered by that hold, then record the next exact user answer before
...
reference for routine progression; it calls the existing evidence-consumption
operation
