...
2. Create a dedicated documentation-sync worker, normally using the advisory
   `technical_writer` profile. Select a ready documentation node whose scope
   includes the affected knowledge surfaces and whose prerequisites bind the
   relevant finalized evidence and artifact generation. The graph, not a second
   textual instruction contract, determines ownership and terminal kind.
...
7. When the active tool returns a current contained digest-verified final
   ...
   closure, governance, handoff, index, and timeline records are
...
creates a bounded ready documentation-impact assignment with the needed
predecessors, waits, and reads its finalized rationale. The coordinator never
publishes on behalf of a worker or self-asserts a no-impact conclusion. It
reconciles the finalized evidence and presents verified links in the mandatory
post-result closure review. The ledger derives closure coverage from the current
graph and reports; no separate initiative or caller-built evidence bundle exists.
For high-risk/material or explicitly reviewed work, missing or unconsumed
required documentation-impact records keep the evidence incomplete until the
coordinator performs bounded documentation rework. Ordinary minimal/light work
still never requires a documentation edit for a no-impact report and never
blocks an honest user-facing explanation of a genuine external limitation.
