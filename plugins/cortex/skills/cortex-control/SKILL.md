...
2. Before dispatching a subagent, call `get_task_status`, then `record_delegation` with its current `expected_revision`. Record the exact profile, task kind, risk, requested model and reasoning effort, ownership, allowed paths, context-file/report references, acceptance criteria, and verification responsibilities. The returned attempt is `awaiting_host_spawn`: it is not running and must not be treated as a confirmed worker. Immediately call the native host `spawn_agent` tool with `spawn_request.task_name`, `spawn_request.message`, selected model, and selected reasoning effort. `spawn_request.task_name` is always the exact canonical profile name (for example, `explorer`); the ledger's separate `attempt_id` is the unique correlation key. After the host returns its real child identifier, call `confirm_host_spawn` with the fresh revision, attempt id, returned host agent id, the same task name, and the actual `host_model` and `host_reasoning_effort` reported by the host. `host_model` is mandatory for model-routed attempts: an omitted value produces a recoverable `host_model_required` response and must be retried. Cortex compares it to the requested model; a mismatch (for example, requested Luna but host Terra) terminalizes the attempt as failed with `host_model_mismatch`, and the worker must never be reported as a successful Luna dispatch. If an adapter returns a different label, Cortex preserves the canonical recorded task name and reports `task_name_correction` rather than failing. A worker report that races ahead of confirmation is accepted with `host_confirmation_pending: true`; evidence and gate completion still wait for confirmation. Only model-verified confirmation makes the ledger attempt `running` and correlates it to the host result. If native spawn fails, use `finalize_attempt` with `failed` or `cancelled` and a reason; never invent an agent id or continue as though a worker existed. `spawn_request.message` includes the exact profile instructions, so adapters without a `profile`, `display_name`, or `title` argument still receive the Cortex role. Do not claim that a visible worker thread was renamed when an adapter such as `multi_agent_v1` exposes no title field. Workers remain hidden and must route every clarification, escalation, blocker, and handoff back to the main chat.
...
7. Record only non-sensitive task metrics when the repository explicitly needs them: scope, checks, verdict, material risk, and durable-artifact location. Do not add token counts, model identifiers, raw prompts, or private telemetry. The model and reasoning effort required in a delegation record remain part of that delegation contract, not general metrics. Task state and gate receipts are authoritative.
   Internal worker communication is English-only: worker prompts, tool arguments, reports, findings, question records, handoffs, and audit events must be written in English. The coordinator alone owns user-facing language and must use the language of the task (or an explicit `user_language`) when presenting questions, blockers, decisions, and summaries. It may pass `localized_question`, `localized_options`, and related display fields to `cortex.question`; those fields never replace the durable English worker record.
...
- Luna handles reading, discovery/data gathering, CRUD-level edits, and small
  fixes with low or moderate risk, independently for every delegation;
- every other non-security dispatch uses `gpt-5.6-terra`, including
  architecture, migration, debugging, and implementation work;
...
Requested reasoning effort `none` normalizes to `low`.
