...
2. Call `classify_task` once with complexity, concrete requirements, the full gate proposal selected by the main orchestrator in `pipeline`, and ordered independent waves in `parallel_groups`, then call `init_task` once with its `classification_id`, objective, and the current `principal`/`thread_id`. Gates in one wave may run concurrently only when dependencies, paths, and resources do not conflict; the next wave waits for all gates in the current wave. The orchestrator owns all optional gates; Cortex appends only the mandatory `documentation` and `close` gates. The receipt is the authoritative complexity, requirements, initial pipeline, and waves contract. Duplicate `complexity`, `requirements`, `pipeline`, or `parallel_groups` fields sent to `init_task` are ignored; initialization always consumes the receipt values and reports corrections when supplied values differ. This creates a private durable ledger task and binds mutations to that principal.
   ...
   Identity is not interchangeable: coordinator calls use the exact bound `principal` and `thread_id` from activation/status, never a worker profile, native child/thread id, or guessed `/root` value. If the host turn does not know the bound thread, omit `thread_id` so the server can restore it; do not retry an identity error with a different value. A worker may use its confirmed host id only when publishing its own unambiguous report; this does not authorize task mutations.
   ...
   Profile aliases such as `performance`, `accessibility`, `ux`, `security`, and `qa` are normalized to their canonical profile ids at the MCP boundary. Coordinators should still prefer canonical ids, but an alias must not be retried as a new attempt.
2. Before dispatching a hidden subagent, inspect the native host `spawn_agent` tool and pass its exact models as `available_models` to `record_delegation`; also inspect `create_thread` and pass its exact models as `available_thread_models` whenever the route can resolve to Luna. Before dispatch, take the model and reasoning effort the orchestrator would otherwise request, normalize `none` to `low`, and apply exactly this remapping table: `gpt-5.6-terra` + `low` → `gpt-5.6-luna` + `high`; `gpt-5.6-terra` + `medium` → `gpt-5.6-luna` + `high`; `gpt-5.6-terra` + `high` → `gpt-5.6-luna` + `xhigh`; `gpt-5.6-sol` + `low` → `gpt-5.6-terra` + `xhigh`; `gpt-5.6-sol` + `medium` → `gpt-5.6-terra` + `max`. For every other model/effort pair, preserve the resolved model and effort unchanged. The table is authoritative even for security or authorized Sol routes; do not apply a broader substitution. If the target model is unavailable, use the documented host-capability fallback and record the actual worker and effort rather than claiming the remapped route. When the hidden host lacks Luna but `create_thread` exposes it, Cortex defaults to a visible `create_thread` Luna task rather than a Terra subagent. A direct `dispatch_mode: visible_thread` remains available when the user wants a visible task unconditionally; `luna_fallback: terra` is an explicit compatibility opt-out. Preserve the selected effort and model unless one of the five table entries applies. Call `list_projects` first for repository work and honor `spawn_request.thread_environment` when creating a visible repository task: pass `target.environment = {type: "local"}` for `local` (the default) or `{type: "worktree"}` only when the request explicitly selects it. Local uses the saved checkout, so serialize writers and concurrent tasks that can modify the same files; worktree remains the isolation option. The attempt remains `awaiting_host_spawn` until the host returns a real child/thread id. Confirm with `confirm_host_spawn`, providing `host_tool: create_thread` and its `threadId` as `host_agent_id` for a visible task, or `host_tool: spawn_agent` and the child id for a hidden worker, plus actual model and effort. The ledger checks both tool and model; an omitted model is recoverable and a mismatch terminalizes the attempt. Visible tasks are user-owned sidebar items; monitor them through `wait_threads`, inspect with `read_thread`, and send context with `send_message_to_thread`. For either route, an adapter label difference is a `task_name_correction`, a racing report remains pending confirmation, and a native creation failure must be finalized as failed or cancelled with its reason. The `spawn_request.message` prompt contains the exact profile instructions, so an adapter such as `multi_agent_v1` exposes no title field without changing the canonical profile. Worker work, reports, questions, and handoffs remain English-only and the coordinator alone translates user-facing results.
...
5. Every worker should call `record_report` with exactly `summary`, `findings`, `questions`, `changed_files`, `tests`, `evidence`, `uncertainty`, and `next_action`; an empty list is required when a list has no entries. Reuse a stable `submission_id` only for a byte-identical retry. If the report changes, use a new lowercase id. A worker must stop when its attempt is terminal/invalidated or has been reworked; it must not publish a late report under another attempt id. For C2/C3, consume that attempt's one-use report receipt with `record_evidence` or `execute_verification_command`. The worker receives the receipt when publishing; the coordinator retrieves it by calling `get_delegation_reports` for the producer attempt and reading `reports[].receipt.receipt_id`. A downstream context grant exposes report bodies but does not transfer their receipts. If an interrupted coordinator has only the exact owned `report_id`, it may supply that as the `report_receipt` hint and Cortex will resolve it only when task, gate, attempt, and active receipt all match. Never guess receipt formats. Link evidence only to `running` or `passed` attempts on the current gate; failed, blocked, cancelled, and superseded attempts do not receive evidence. Command evidence must include `exit_code`; use `execute_verification_command` with one of the fixed catalog `verification_id` values whenever command proof matters. A C2/C3 close requires at least one successful server-observed command (exit 0), not a self-attested command. Then call `record_gate_outcome`. A passed attempt must have linked evidence; terminal non-success attempts (`failed`, `blocked`, `cancelled`, or `superseded`) may lack a report when the coordinator records an explicit reason. Mandatory documentation/close skip requests return a structured recoverable response directing the coordinator to delegate the gate. A skipped gate advances without a pass claim. `blocked` preserves the gate and marks the task blocked; create the current-gate handoff before recording that blocked outcome, then use `resume_task` after the blocker is resolved.
...
5. If repository evidence changes scope, call `reassess_pipeline` with typed `intent` (`add_specialist`, `resequence`, `rework_gate`, or `stop`) and new signals. For a pipeline decision, first preview with `apply: false`, `decision: unchanged`, and the current revision; inspect the returned operations and suggested waves. Then apply only the reviewed replacement with the returned revision, `apply: true`, and `decision: updated`. If no operations are proposed, keep `decision: unchanged`; never send `decision: updated` with `apply: false` or omit the decision on C2/C3. Removing a completed gate requires `allow_rework: true`; without it the existing evidence is protected. A `rework_gate` invalidates its old evidence and requires all old attempts to be terminal before a replacement is dispatched. Replanning is bounded by the task retry budget; after exhaustion, stop and hand off instead of dispatching another attempt. `update_pipeline` supports atomic `add`, `remove`, `move`, `replace`, and explicit `rework` operations. Rework resets completion without deleting the gate or its prior attempts. The original task definition remains immutable.
...
- a read-only profile alone does not imply Luna; non-analysis work initially
  uses `gpt-5.6-terra`, including architecture, migration, debugging, and
  implementation work, then follows the exact model/effort remapping table;
- security task kind, the security gate, and the `security_auditor` profile
  initially resolve to `gpt-5.6-sol`, normalizing contradictory task kinds to
  security, then follow the exact model/effort remapping table; and
