...
## 9.2.7 release candidate

This is a source-tree hardening candidate, not a published release. Its source
cachebuster is `9.2.7+codex.20260820104507`. Full-suite, live-governance,
...
- The plugin and MCP server versions must match the release contract
  `9.2.7` (the current source candidate is
  `9.2.7+codex.20260820104507`; installed builds may carry a cachebuster).
...
- Create the Cortex 9.2.7 release commit only with explicit authorization.
