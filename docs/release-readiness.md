...
cachebuster is generated from the 9.2.22 base version. Full-suite, live-governance,
...
The 9.2.22 patch keeps programmatic dispatch-briefing size rejection removed and
...
The bundled coordinator contract and public MCP description now make the
start-task boundary explicit: `task.verification` is an array of concrete
authoritative checks, while `verification_mode` is unsupported. Unknown task
fields remain rejected before task reservation or creation.
