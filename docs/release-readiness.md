...
## 9.2.8 release candidate

This is a source-tree hardening candidate, not a published release. Its source
cachebuster is generated from the 9.2.8 base version. Full-suite, live-governance,
...
- The plugin and MCP server versions must match the release contract
  `9.2.8` (the current source candidate carries a cachebuster; installed builds
...
- Create the Cortex 9.2.8 release commit only with explicit authorization.
