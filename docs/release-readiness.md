...
- The plugin and MCP server versions must match the release contract
  `9.1.1` (the current candidate is
  `9.1.1+codex.20260819034734`; installed builds may carry a cachebuster).
...
The 9.1.1 source candidate passed the fresh 517-test suite, marketplace
...
focused regression coverage. The 9.1.1 ledger starts from SQLite only: its
...
- Create the Cortex 9.1.1 release commit only with explicit authorization.
