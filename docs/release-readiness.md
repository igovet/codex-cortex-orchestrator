...
- The plugin and MCP server versions must match the release contract
  `9.1.0` (the current candidate is
  `9.1.0+codex.20260818230853`; installed builds may carry a cachebuster).
...
The 9.1.0 source candidate passed the fresh 514-test suite, marketplace
...
focused regression coverage. The 9.1.0 ledger starts from SQLite only: its
...
- Create the Cortex 9.1.0 release commit only with explicit authorization.
