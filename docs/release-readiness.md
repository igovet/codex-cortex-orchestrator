...
- The plugin and MCP server versions must match the release contract
  `3.3.0` (an installed build may carry a `+codex.<build>` suffix).
...
Current 3.3.0 evidence includes 237 passing tests, changed-skill/plugin/
marketplace validation, compilation and shell syntax, installed and
content-verified cachebuster `3.3.0+codex.20260814224159`, installer
check/dry-run, cold boot, deterministic fixtures, composite benchmark, and the
isolated fresh-plugin probe. Details are recorded in
`docs/project/verification.md`. Live-model and tracked-release validation
remain separate release gates; historical 3.2.x results do not attest this
candidate.
...
- Create the Cortex 3.3.0 release commit only with explicit authorization.
