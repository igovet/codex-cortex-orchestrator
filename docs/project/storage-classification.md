...
| `backups/<name>.cortex-backup/{cortex.db,governance-lifecycle.key,manifest.json}` | Explicit maintenance disaster-recovery bundle | Confirmed backup route writes; verified restore route reads | Optional, only after explicit `BACKUP` confirmation | Private secret-bearing bundle: directory `0700`, members `0600`, no symlinks and current-user ownership required. It atomically preserves the SQLite ledger together with the v12 governance lifecycle key and fingerprints; verify restores both into a fresh disposable host root and authenticates governance records. Never attach, log, or treat a bare historical `.sqlite` snapshot as a recoverable Cortex backup. |
