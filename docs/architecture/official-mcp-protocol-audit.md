...
| `tools/list` | Returns tools and supports opaque pagination cursor; list-change notification is optional | Immutable audience-specific projection with opaque cursor handling; no list-change capability | Pass; coordinator and worker projections require separate black-box checks |
