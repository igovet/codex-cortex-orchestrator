...
development-only support files. No runtime guarantee may depend on this root
`AGENTS.md`; tests must prove the equivalent contract is bundled below
`plugins/cortex/` for use in every installed project.
...
- Preserve the selected profile's exact `name` as the machine-readable role. Use Cortex's returned `display_name` in human-readable `Profile Module` form (for example, `Explorer Auth`) as the label, and its exact returned task/attempt-unique native `task_name`—which carries the ordinal and uniqueness digest—when the creation API exposes only a restricted task-name field. Never invent or paraphrase either value.
...
- Every native dispatch carries only a compact bootstrap plus `dispatch_ref`, an exact immutable `briefing_path`, and `briefing_digest`; the full worker prompt must not be embedded in the MCP result or mutable task/delegation state. The worker reads only that exact read-only briefing, verifies its SHA-256 before project work, never lists or directly reads any other `.codex/cortex` path, and includes the exact bootstrap-supplied `Dispatch briefing reviewed: <sha256>` item in report evidence. If the host filesystem read alone cannot open that exact path, the worker may call public `read_dispatch_briefing` with the complete identity/digest tuple from its bootstrap; if its bounded result is incomplete, it may continue only with the returned cursor until complete. A failure remains a blocker and never authorizes directory access or another briefing. `record_report` revalidates the file, digest, permissions, and marker. Predecessor reports and other durable coordination artifacts remain accessible only through their scoped Cortex tools. The coordinator must not read, inline, expand, or reconstruct worker briefing files.
...
- Every worker writes its complete seven-field `cortex/report/v1` through the scoped `record_report` tool. Its native final response contains only `REPORT_RECORDED report_ref=<value>` plus at most a two-sentence summary, or the exact report-tool error. The coordinator reads refs with `read_worker_report` and advances with `report_ref`; a successor worker may also call `read_worker_report` with its exact attempt/profile only for predecessor refs explicitly supplied in its dispatch. Workers must never paste large report JSON into the parent channel.
- `followup_task` is authorized only after a durable worker question has been answered. A native final report-tool error or any completion other than `REPORT_RECORDED`/`QUESTION_RECORDED` means `SubagentStop` has already classified the attempt: inspect once, then recover its report/question or submit the exact failed result. Never send corrective follow-up to that stopped child; only a fresh top-level Cortex dispatch authorizes rework.
...
- After context compaction, clear, or resume, preserve `task_ref` and call `manage_orchestration(intent="inspect")` exactly once. Rehydrate from `context_handoff`: invoke only top-level inspect dispatches matching `pending_dispatches`, never spawn from the handoff itself, never respawn `active_workers`, and wait only while their exact persisted `host_agent_id` values remain running. Never wait on or respawn `stopped_workers`: consume a persisted report, surface a durable question, or submit the exact failed result for rework. `SubagentStart` must bind the documented native `agent_id`, task key, and actual model to the attempt; `SubagentStop` must durably distinguish report-recorded, question-paused, and failed-without-report outcomes. A running attempt without durable binding fails closed instead of permitting a duplicate spawn or an unbound wait.
...
- Read-only workers must choose non-writing verification modes before execution: use `PYTHONDONTWRITEBYTECODE=1` for Python, disable pytest and equivalent test/build caches, and skip checks that require cleanup. They never create an artifact and then invoke `rm`, `git clean`, or a cleanup script; Cortex compares generated/gitignored artifact sentinels as well as ordinary project files against the attempt baseline.
