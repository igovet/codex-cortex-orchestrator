...
- The repository-root `./cortex-dev` command is a convenience forwarding
  ...
  constraints.
- Real Desktop live-dev uses `./scripts/cortex-desktop-dev`. It must prepare the
  same isolated candidate through `./scripts/cortex-dev --prepare-only`, launch
  the actual Desktop binary with a disposable Electron profile, and never
  modify the stable Codex profile or stable Cortex plugin. CLI/Desktop parity
  requires consecutive successful real-host scenarios on one unchanged
  cache-stamped payload, in either order. Any installable-payload edit
  invalidates both earlier live results and requires the two-host pair again.
...
- The focused E2E acceptance scenario is multi-turn and LLM-driven: in a separate test project, let ordinary low-risk work continue without a pre-plan question; when the plan is high-risk/material (or approval was explicitly requested), approve the visibly rendered plan packet, including any branch or API-key/ENV prerequisite, and continue planner → implementation → independent verification → documentation-impact assessment → closure. Inspect every native worker's bounded structured event stream, including the first report-submission event; any hidden tool error or unexplained replay fails the scenario. The transport only delivers text/keys and exposes observations; it never asks a clarification, approves a plan, or decides acceptance autonomously.
